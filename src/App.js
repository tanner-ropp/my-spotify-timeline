...
class App extends Component {
  constructor(props) {
    ...
    if (
      params.access_token &&
      params.state === window.sessionStorage.getItem("auth_state")
    ) {
    ...
    if (this.state.loggedIn) {
      ...
      spotifyWebApi
        ...
        .then((response) => {
          ...
          const prevResponse = response.artists.items.map((artist) => {
            return { name: artist.name, id: artist.id };
          });

          return this.getNextArtists(
            prevResponse,
  ...
  getNextArtists(prevResponse, offsetId, next) {
    if (next) {
      return spotifyWebApi
        .getFollowedArtists({ limit: 50, after: offsetId })
        .then((response) => {
          const _prevResponse = prevResponse.concat(
  ...
  requestAuthorization() {
    ...
    const scope =
      "user-read-private user-read-email user-follow-read user-read-playback-state user-modify-playback-state";
  ...
  fetchData() {
    ...
    this.setState({
      loadedReleases: this.state.loadedReleases.concat(
        this.state.artistReleases.slice(oldLength, oldLength + 10)
      ),
    });
  }
  ...
  render() {
    ...
    const releaseList = loadedReleases.map((release) => {
      return <ReleaseCard key={release.id} releaseInfo={release} />;
    });
            ...
            <header className="stickyHeader">
              ...
              <div className="header-profile">
                ...
                <div className="imageCropper">
                  <img src={this.state.profileImage} height="50px" />
                </div>
              </div>
            </header>
            <div className="body">
